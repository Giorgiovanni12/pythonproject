...
class Application(tk.Tk):
    def __init__(self):
        ...
        self.background_image = PhotoImage(file="/home/giorgio/EsameFinalePY/Main/building_with_python_watermark (1).png")
        ...
        # Creare i bottoni per le due azioni
        ...
        self.theme_checkbutton.pack(pady=8)
...
#funzioni per la libreria tkinter
...
def transcribe_audio_file(audio_file_path):
    ...
    with sr.AudioFile(audio_file_path) as sorgente:
        audio_data = recognizer.record(sorgente)
                                                                #funzione per trascrivere il file 
    try:
        text = recognizer.recognize_google(audio_data)
        return text
    except sr.UnknownValueError:
        print("Non si riesce a comprendere il contenuto del file")  #gestione degli errori
    except sr.RequestError as e:
        print(f"Speech Recognition non accetta la richiesta :( {e}")

def summarize_text(text):
    text = re.sub(r'http\S+', '', text)    #Prompt di OpenAi con lo scopo di riassumere il seguente testo,non prendendo in considerazione eventuali link
...
def split_text_into_lines(text, massimoparole):     #funzione per dividere le parole e andare a capo con costanza
    words = text.split()
    linee = []
    current_line = []
    ...
    for word in words:
        ...
        if len(current_line) == massimoparole:
            linee.append(" ".join(current_line))
            current_line = []
